    ...
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            ...
            case 'edit':
                ...
                if (mysqli_query($conn, $sql)) {
                    $_SESSION['success'] = "แก้ไขแผนกสำเร็จ";
                } else {
                    $_SESSION['error'] = "เกิดข้อผิดพลาด: " . mysqli_error($conn);
                }
                break;
            ...
            case 'delete':
                ...
                // ตรวจสอบว่ามีพนักงานหรือครุภัณฑ์ในแผนกนี้หรือไม่
                $check_sql = "SELECT 
                    (SELECT COUNT(*) FROM users WHERE department_id = $id) as user_count,
                    (SELECT COUNT(*) FROM equipment WHERE department_id = $id) as equipment_count";
                $check_result = mysqli_query($conn, $check_sql);
                $counts = mysqli_fetch_assoc($check_result);
                
                if ($counts['user_count'] > 0 || $counts['equipment_count'] > 0) {
                    $_SESSION['error'] = "ไม่สามารถลบได้ เนื่องจากมีพนักงานหรือครุภัณฑ์ในแผนกนี้";
                } else {
...
<head>
    ...
    <title>จัดการแผนก - ระบบจัดการครุภัณฑ์</title>
    ...
    <div class="d-flex">
        ...
        <div class="sidebar" style="width: 280px;">
            <div class="p-3">
                ...
                <a href="departments.php" class="sidebar-item active">
                                    ...
                                    <td>
                                        <div class="stat-info">
                                            <div class="stat-item">
                                                ...
                                                <span><?php echo number_format($row['user_count']); ?> คน</span>
                                            </div>
                                            <div class="stat-item">
                                                <i class="fas fa-desktop"></i>
                                                <span><?php echo number_format($row['equipment_count']); ?> รายการ</span>
                                            </div>
                                        </div>
                                    </td>
