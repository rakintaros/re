...
if (isset($_GET['export']) && $_GET['export'] == 'excel') {
    ...
    // สถิติภาพรวม
    echo '<h2>รายงานระบบครุภัณฑ์ - ' . date('d/m/Y H:i:s') . '</h2>';
    echo '<table border="1" cellpadding="5">';
    echo '<tr><th colspan="2" bgcolor="#ff6b35" style="color:white;">สถิติภาพรวม</th></tr>';
    echo '<tr><td>ครุภัณฑ์ทั้งหมด</td><td>' . number_format($overview['total_equipment']) . ' รายการ</td></tr>';
    echo '<tr><td>ครุภัณฑ์ปกติ</td><td>' . number_format($overview['normal_equipment']) . ' รายการ</td></tr>';
    echo '<tr><td>กำลังซ่อม</td><td>' . number_format($overview['repairing_equipment']) . ' รายการ</td></tr>';
    echo '<tr><td>ชำรุด</td><td>' . number_format($overview['damaged_equipment']) . ' รายการ</td></tr>';
    echo '<tr><td>การแจ้งซ่อมทั้งหมด</td><td>' . number_format($overview['total_repairs']) . ' ครั้ง</td></tr>';
    echo '<tr><td>ซ่อมเสร็จแล้ว</td><td>' . number_format($overview['completed_repairs']) . ' ครั้ง</td></tr>';
    echo '<tr><td>รอดำเนินการ</td><td>' . number_format($overview['active_repairs']) . ' ครั้ง</td></tr>';
    echo '</table><br><br>';
    
    // รายละเอียดการซ่อม
    echo '<table border="1" cellpadding="5">';
    echo '<tr bgcolor="#ff6b35" style="color:white;">';
    echo '<th>เลขที่แจ้งซ่อม</th>';
    echo '<th>วันที่แจ้ง</th>';
    echo '<th>รหัสครุภัณฑ์</th>';
    echo '<th>ชื่อครุภัณฑ์</th>';
    echo '<th>ประเภท</th>';
    echo '<th>ผู้แจ้ง</th>';
    echo '<th>แผนก</th>';
    echo '<th>ทีมซ่อม</th>';
    echo '<th>ช่างผู้รับผิดชอบ</th>';
    echo '<th>สถานะ</th>';
    echo '<th>ความเร่งด่วน</th>';
    echo '<th>ระยะเวลา</th>';
    echo '</tr>';
    ...
    while ($row = mysqli_fetch_assoc($result_export)) {
        ...
        echo '<td>' . $status_thai[$row['status']] . '</td>';
        echo '<td>' . $urgency_thai[$row['urgency']] . '</td>';
        echo '<td>' . $duration . '</td>';
        echo '</tr>';
    }
        ...
        <div class="flex-grow-1 content-area">
            ...
            <div class="page-header">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <a href="?export=excel" class="btn btn-light">
